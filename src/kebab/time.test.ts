...
import { formatUtcInTimeZone, HR_TIME_ZONE, parseLocalTimeToUtc } from "./time";

describe("time helpers (Luxon)", () => {
  it("converts Croatia-local time to UTC (winter)", () => {
    const res = parseLocalTimeToUtc("2026-03-15", "12:00", HR_TIME_ZONE);
  ...
  it("converts Croatia-local time to UTC (summer)", () => {
    const res = parseLocalTimeToUtc("2026-04-15", "12:00", HR_TIME_ZONE);
    expect(res.ok).toBe(true);
    if (!res.ok) return;
  ...
  it("rejects spring-forward gap times (DST)", () => {
    ...
    const res = parseLocalTimeToUtc("2026-03-29", "02:30", HR_TIME_ZONE);
  ...
  it("resolves fall-back ambiguous times consistently (DST)", () => {
    ...
    const a = parseLocalTimeToUtc("2026-10-25", "02:30", HR_TIME_ZONE);
    const b = parseLocalTimeToUtc("2026-10-25", "02:30", HR_TIME_ZONE);

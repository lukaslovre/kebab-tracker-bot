...
export type KebabDashboardReplyData = {
  rating: number | null;
  isBackdated: boolean;
  /** Display timestamp in the configured local timezone as `YYYY-MM-DD HH:mm`. */
  eatenAtLocal: string;
...
function renderCommandExamples(trackerCommand: string): string[] {
  return [
    `- \`${trackerCommand}\``,
    `- \`${trackerCommand} 8/10\``,
...
export function renderKebabDashboardReply(
  ...
  if (data.isBackdated) {
    ...
    lines.push(`📅 *Retroaktivno:* ${data.eatenAtLocal}`);
  }
  ...
  lines.push("");
  lines.push(
    `^(_Za retroaktivni unos: \`${options.trackerCommand} YYYY-MM-DD\` ili \`${options.trackerCommand} YYYY-MM-DD HH:mm\`_)`,
  );
...
export function renderKebabCooldownReply(options: {
  ...
  return [
    "⏳ **Polako!**",
    "",
    `Možeš logirati novi kebab nakon **${options.nextAllowedAtLocal}**.`,
    "",
    "Primjeri:",
...
export function renderKebabFutureDateReply(options: {
  trackerCommand: string;
}): string {
  return [
...
export function renderKebabParseErrorReply(options: {
  message: string;
  trackerCommand: string;
}): string {
  return [
    `❓ **Ne kužim.** ${options.message}`,
    "",
    "Primjeri:",
    ...renderCommandExamples(options.trackerCommand),
    `- \`${options.trackerCommand} 8/10 2026-03-15 18:30\``,
